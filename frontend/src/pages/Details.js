...
import { Button, Center, Flex, Input, Box, Badge, Text, Divider, chakra } from '@chakra-ui/react'
        ...
        <Flex flexDir={'column'} alignItems='stretch' alignSelf='center' justifyContent='start' w='90vw' h='100%' backgroundColor='#6e0707'   m={5}>
            {!loading && !error && data && data.subnet_id && (
                ...
                <Flex flexDir='row' justifyContent='space-between' backgroundColor='white' shadow='0 0 10px 2px white' color='black' p={6} rounded='2xl'>
                    <Flex flexDir='column'> 
                            {data.alias && <Text fontSize='2xl' fontWeight='bold'>{data.alias}</Text>}
                            <Text fontSize='xl'>Subnet ID: {data.subnet_id}</Text>
                    </Flex>
                    {data.alias ? (
                ...
                <Flex mt={6} alignItems='center' flexDirection='column' maxHeight='80vh' overflowY='scroll'>
                    <Text textAlign='left' color='white' alignSelf='start' ml={20} fontSize='24px' fontWeight='bold'>Validators</Text>
                    <Divider />
                        ...
                        <Flex flexDir='column' py={1} px={3} my={3} key={validator.node_id} w='85%' backgroundColor='gray.200' shadow='0 0 3px 2px white' color='black' rounded='lg'>
                            <Flex flexDir='row' justifyContent='space-between'>
                                <Flex flexDir='column'>
                                    ...
                                    <Text fontSize='sm'>Tx ID: {validator.tx_id}</Text>
                                </Flex>
                                <Text fontSize='sm'>Uptime: {validator.uptime ? <span style={{color: validator.uptime * 100 + '%' < 75 ? 'red' : 'green'}}><b>{String(validator.uptime * 100).slice(0,4) + '%'}</b></span> : <span>Undefined</span>}</Text>
                            </Flex>
                            <Flex justifyContent='stretch' flexDirection='column' width='100%' mt={1}>
                                ...
                                <Flex justifyContent='stretch' width='100%' my={1}>
                                    <Box w={handleWidth(validator.start_time, validator.end_time)[0]} h='3px' bg='red.500' shadow='0 0 3px 1px red' rounded='lg' roundedRight='none' />
                                    <Box w={handleWidth(validator.start_time, validator.end_time)[1]} h='3px' bg='red.900' rounded='lg' roundedLeft='none' />
                                </Flex>
                            </Flex>
                        </Flex>
                    ))}
                    {data.validators && data.validators.length === 0 && <Text fontSize='24px' pt={20} fontWeight='bold' color='white'>No validator is validating this subnet!</Text>}
                </Flex>
                </>
            )}
